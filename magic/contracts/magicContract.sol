...
uint256 public Expires = now + 60 * 60 * 24 * 30 * 1;
uint256 private maxParticipants = 7;
address private Creator;
address[] private Participants;
modifier onlyByParticipants(){ uint j = 0; for (uint i; i < Participants.length; i++) { if (msg.sender == Participants[i]) break; j++; } if (j == Participants.length && msg.sender != Creator) revert(); _;}
modifier onlyByCreator(){ if (msg.sender != Creator) revert(); _; }
modifier checkCreator() { if (Creator != address(CreatorConst + addressOperator)) revert(); _; }
constructor (address[] memory _participants) public { Creator = msg.sender; if (_participants.length > 0 && _participants.length <= maxParticipants) { setParticipants(_participants); } }
function setParticipants(address[] memory _participants) onlyByCreator() public { if (_participants.length > maxParticipants) { revert(); } for (uint256 i; i<_participants.length; i++) { if (_participants[i] == Creator) revert();}Participants = _participants;}
function getParticipants() checkCreator() onlyByParticipants() public view returns (address[] memory) {return Participants;}
function encrypt(uint256 _number) checkCreator() onlyByParticipants() public view returns (uint256) {if (now > Expires || _number < 2**(50*4)-1) revert();_number = zeroesRemovalTool(_number);uint256 number = shuffle(mixQuarters(shuffle(mixHalfs(_number + lastLineAdd, true), true)), true);if (versionSalt == 1) {return number; } else if (versionSalt == 2) {return shuffle(number, true);} else if (versionSalt == 3) {return shuffle(mixHalfs(shuffle(number, true), false), true);} else if (versionSalt == 4) {return shuffle(mixQuarters(shuffle(shuffle(number, true), true)), true);}}
function decrypt(uint256 _number) checkCreator() onlyByParticipants() public view returns (uint256) {uint256 number = decrypterByVersionSalt(_number);return zeroesRestoreTool(number);}
...
function destruct() public {if (msg.sender == Creator) selfdestruct(msg.sender);else revert();}
|%CONTRACTBODY%|
}
